...
public:
  Bone(const std::string &name, int ID, const aiNodeAnim *channel)
    ...
    for (int positionIndex = 0; positionIndex < m_NumPositions; ++positionIndex) {
      ...
      KeyPosition data;
      data.position  = AssimpUtils::GetGLMVec(aiPosition);
    ...
    for (int rotationIndex = 0; rotationIndex < m_NumRotations; ++rotationIndex) {
      ...
      KeyRotation  data;
      data.orientation = AssimpUtils::GetGLMQuat(aiOrientation);
      data.timeStamp   = timeStamp;
    ...
    for (int keyIndex = 0; keyIndex < m_NumScalings; ++keyIndex) {
      ...
      KeyScale   data;
      data.scale     = AssimpUtils::GetGLMVec(scale);
      data.timeStamp = timeStamp;
...
private:
  ...
  glm::mat4 InterpolateRotation(float animationTime) {
    ...
    int   p1Index     = p0Index + 1;
    float scaleFactor = GetScaleFactor(
        m_Rotations[p0Index].timeStamp, m_Rotations[p1Index].timeStamp, animationTime);
